...
For this to work, you need to create a Kubernetes Secret containing the kubeconfig for the target cluster. The Secret should have the label `vidra-kubeconf` with the URL of the target cluster. Below is an example of how to create this Secret:
...
metadata:
  ...
  labels:
    vidra-kubeconf: <cluster-url-without-http>
type: Opaque
data:
...
## Creating a `VidraResource` Resource
...
To manage resources using the Vidra Operator, you can create a `VidraResource` resource. Below is an example of how a `VidraResource` resource looks like:
...
A `VidraResource` resource only requires the `spec.manifest` field, which should contain the Kubernetes manifests you want Vidra to manage.

A `VidraResource` resource created by `InfrahubSync` and a completed reconciliation will look like this:
```yaml
apiVersion: infrahub.operators.com/v1alpha1
kind: VidraResource
metadata:
