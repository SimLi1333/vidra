...
An `InfrahubResource` is a Kubernetes CR created and managed by the Vidra Operator based on the results of an `InfrahubSync`. Each `InfrahubResource` corresponds to a specific resource from Infrahub and contains:
...
The `Manifest` itselfe is not saved in the domain model, but rather in the `InfrahubResource` CR. The domain model focuses on the core logic and operations related to artifacts. 
